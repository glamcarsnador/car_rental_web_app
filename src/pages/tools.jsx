  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-bold text-main tracking-tight">{t('tools')}</h1>
      </div>
      
      {/* Skeleton Content */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <section className="space-y-4">
          <div className={cn("h-8 w-40 bg-module rounded", pulseClass)} />
          <div className="grid grid-cols-1 gap-4">
             {[1, 2, 3].map((i) => (
              <div key={i} className={cn("h-24 bg-module/50 rounded-xl border border-border", pulseClass)} />
            ))}
          </div>
        </section>
        <section className="space-y-4">
