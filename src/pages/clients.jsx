  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-bold text-main-text tracking-tight">{t('clients')}</h1>
      </div>
      
      {/* Skeleton Content */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className={cn("h-48 bg-module-bg/50 rounded-xl border border-border-main", pulseClass)} />
        ))}
      </div>
    </div>
  );
